class Pflanze4 extends Pflanze
{
  static final float PHI = PI/7;
  static final float startLength = 40;
  static final float x_offset = 20;
  static final float y_offset = 100;
  float drawLength;
  public static final float breite = 2*x_offset;
  public static final float hoehe = y_offset;
  
  public Pflanze4(int x, int y, int b, int h, String farbe, int age)
  {
    super(x,y,b,h,farbe);
    this.age = age;  
  ...
  {
    this.axiom  = "F";
    this.grammar = new StochasticGrammar(axiom);  // initialize library
    this.grammar.addRule('F', "F[+F]F[-F]F", 0.1); // add rule, and weight
    this.grammar.addRule('F', "F[+F]F", 0.45);
    this.grammar.addRule('F', "F[-F]F", 0.45);
